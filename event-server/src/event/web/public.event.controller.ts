...
import {
  ...
  RewardItemDto,
} from './dto/event-rewards.response';
...
export class PublicEventController {
  ...
  async getEvents(
    ...
    const { totalCount, items } = await this.eventService.getAllEventsWithPage(
      page,
      limit,
    );
  ...
  async getRewardsByEventId(
  ...
  ): Promise<CommonResponse<EventRewardResponseDto>> {
    const event = await this.eventService.getRewardsByEventId(eventId);
    ...
    return new CommonResponse(200, '이벤트 보상 목록 조회 성공', dto);
  }
}
